...
class Lines():
    ...
    def color_thresh(self, img, thresh=(0, 255)):
        ...
        hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HLS).astype(np.float32)
        s_channel = hsv[:,:,2]
    ...
    def binary_extraction(self,image, ksize=3):
        ...
        color_bin = self.color_thresh(image,thresh=(200, 255))             #90,150 # initial values 110, 255

        gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)

        sobelx = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize)
        sobely = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize)
        gradx = self.abs_sobel_thresh(sobelx, thresh=(40, 160))             # initial values 40, 160

        grady = self.abs_sobel_thresh(sobely, thresh=(40, 160))             # initial values 40, 160
    ...
    # performs transform perspective on the image
    def trans_per(self, image):
        ...
        #Adjust these values to fit the best polygon where the lanes appear
        left_bottom = (0,ysize)
        apex_l = (xsize/5,ysize/7)
        apex_r = (xsize*(4/5),ysize/7)
    ...
    def get_fit(self, image):
        ...
        if not self.detected:
            # window settings  >>These values are experimental, try adjusting to fit your best performance
            window_width = 35
            window_height = 35 # break image into 9 vertical layers since image height is 720
        ...
        # if lanes were detected in the last frame, search area for current frame
            ...
            if (len(self.allxl)>0) & (len(self.allxr)>0):
                ...
                if self.check_fit():
                    ...
                    self.best_fit_l = self.best_fit_l*m + self.current_fit_l * n
                    self.best_fit_r = self.best_fit_r*m + self.current_fit_r * n
                    # assign new best values based on this iteration
                    self.bestxl = self.allxl
                    self.bestyl = self.allyl
                    self.bestxr = self.allxr
    ...
    def project_on_road_debug(self, image_input):
        ...
        if self.detected_first & self.detected:
            ...
            img_text = "radius of curvature: " + str(round((self.left_curverad + self.right_curverad)/2,2)) + ' (cm)'

            if self.offset< 0:
                img_text2 = "vehicle is: " + str(round(np.abs(self.offset),2)) + ' (cm) left of center'
            else:
                img_text2 = "vehicle is: " + str(round(np.abs(self.offset),2)) + ' (cm) right of center'
            ...
            lost = False
            return result1,result2, self.offset, self.right_curverad, self.left_curverad,lost
        ...
        # if lanes were not detected output source image
            ...
            return return_image, return_image,0,0,0,lost
...
camera.framerate = 12  #Using a large fps decreases the accuracy> use(min 7, max 28) #Experimental
camera.vflip = False
camera.hflip = False
...
def main(debug=True):
    try:
        for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
            image = frame.array #Capture the image frame
            result1, result2, offset, rightC, leftC, lost = lines.project_on_road_debug(image)

            if lost:
                print("Lost Lanes!!!")
            else:
                print("Offset: ", offset)
                print("Right Curvature: ", rightC)
                print("Left Curvature: ", leftC)
                print()
                cv2.imshow("Rpi Lane Detection",result1)
                if debug:
                    cv2.imshow("Debug Window", result1)
